...
ITSL_count <- ITSLdata[,.(id = 1:dim(ITSLdata)[1],
                          L1TT,
                          L2TT,
                          L3TT,
                          L4TT,
...
ITSL_count <- melt(data = ITSL_count,
                   id.vars = "id",
...
layer_sp <- data.table(expand.grid(id = unique(ITSL_layer$id),
                                   Layer = 1:4,
                                   spn = 1:4))
...
for (indilayer in 1:4){
  sp <- reshape(data = ITSL_layer[,c("id", paste0("L", indilayer, c("sp1", "sp2", "sp3", "sp4"))), with = FALSE],
                idvar = "id",
  ...
  pct <- reshape(data = ITSL_layer[,c("id", paste0("L", indilayer, c("sp1pct", "sp2pct", "sp3pct", "sp4pct"))), with = FALSE],
                 idvar = "id",
  ...
  age <- reshape(data = ITSL_layer[,c("id", paste0("L", indilayer, c("sp1age", "sp2age", "sp3age", "sp4age"))), with = FALSE],
                 idvar = "id",
  ...
  ht <- reshape(data = ITSL_layer[,c("id", paste0("L", indilayer, c("sp1ht", "sp2ht", "sp3ht", "sp4ht"))), with = FALSE],
                idvar = "id",
