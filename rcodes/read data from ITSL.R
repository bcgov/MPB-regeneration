...
ITSLdata <- data.table(read.xlsx(file.path(ITSLdatapath, "BCTS ITSL summary TOR 2020-12-04ch.xlsx"),
                                 sheet = "Scrape",
                                 detectDates = TRUE))
ITSLdata <- ITSLdata[,.(id = 1:dim(ITSLdata)[1],
...
is.element(TRUE, is.na(ITSLdata[,.(Lat)]))
is.element(TRUE, is.na(ITSLdata[,.(Long)]))

pointmap <- SpatialPointsDataFrame(ITSLdata[,.(Long, Lat)],
                                   data = ITSLdata,
...
setnames(ITSLdata,"Lat","GPSlat")
setnames(ITSLdata,"Long","GPSlong")
ITSLdata <- cbind(ITSLdata, bcalbers)

###Wide table to long table

###1. total tallied numbers and crown closures by layers

# The comment code below use sperate(), the uncomment code use melt()
...
ITSLdata_layer <- ITSLdata[,.(id,
                              ...
                              L3CC,
                              L4CC)]
ITSLdata_layer <- melt(data = test,
                       id.vars = "id",
                       measure.vars = list(c("L1TT", "L2TT", "L3TT", "L4TT"), c("L1CC", "L2CC", "L3CC", "L4CC")),
                       variable.name = "Layer",
                       value.name = c("TT", "CC"),
...
####2. species, age, ht by layers

ITSL_layer_sp <- ITSLdata[,.(id,
                             L1sp1,
                             L1sp1pct,
                             L1sp1age,
                             L1sp1ht,
